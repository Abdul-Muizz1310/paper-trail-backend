...
**The gate to ship was concrete.** Twenty-five labelled claims — TRUE, FALSE, and INCONCLUSIVE — form the eval set. Real-mode eval runner hits the live LangGraph against real OpenRouter calls and exits non-zero if accuracy < 80%. The first full run on `main` scored **21/25 = 84%**, with 20/20 correct on clear TRUE/FALSE claims and all four misses concentrated on genuinely contested health claims where the Judge preferred `FALSE` over `INCONCLUSIVE`: red wine longevity, intermittent fasting vs calorie restriction, classical music raising infant IQ, and multivitamins reducing all-cause mortality (rows 22–25 of [`evals/report.md`](evals/report.md)). The fifth `INCONCLUSIVE` claim in the set — moderate coffee consumption — the Judge got *right*. Arguably the Judge was right on the other four too and the ground-truth labels are too lenient, but we measure against labels, so they count as misses.
